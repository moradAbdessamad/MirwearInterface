...
hover_start_time = {}
hover_duration = 2  # Duration to hover in seconds
...
@socketio.on('update_button_positions')
def update_button_positions(data):
    global buttons_positions
    buttons_positions = data  # Update the global button positions
    print(f"Received button positions: {buttons_positions}")
...
def check_button_hover(finger_tip_coords):
    ...
    global buttons_positions  # Use the dynamically updated positions

    if finger_tip_coords and buttons_positions:
        for button in buttons_positions:
            top_left = button['top_left']
            bottom_right = button['bottom_right']

            # Check if the finger is hovering over the button
            if top_left['x'] <= finger_tip_coords['x'] <= bottom_right['x'] and \
               top_left['y'] <= finger_tip_coords['y'] <= bottom_right['y']:

                if button['name'] not in hover_start_time:
                    hover_start_time[button['name']] = time.time()
                elif time.time() - hover_start_time[button['name']] >= hover_duration:
                    # If the button is one of the options
                    if button['name'] in ['option1_1', 'option1_2', 'option1_3']:
                        print(f"{button['name']} is hovered")
                        if current_category and current_items:
                            selected_item = select_item_by_option(current_items, button['name'])
                            ...
                            if selected_item and not request_in_progress_flag:
                                ...
                                threading.Thread(target=send_request, args=(button['name'], selected_item)).start()
                    else:
                        message = {'button': button['name']}
                        print(f"Emitting message: {message}")
                        socketio.emit('button_hover', message)
                    hover_start_time.pop(button['name'])
            else:
                if button['name'] in hover_start_time:
                    hover_start_time.pop(button['name'])


capture_requested_recommand = False
buttons = {
    'Top': {'top_left': (0.01, 0.1218), 'bottom_right': (0.05, 0.164)},  # Left side
    'Bottom': {'top_left': (0.01, 0.356), 'bottom_right': (0.05, 0.423)},  # Left side
    'Foot': {'top_left': (0.01, 0.518), 'bottom_right': (0.05, 0.66)},  # Left side
    'Recommend': {'top_left': (0.95, 0.517), 'bottom_right': (0.99, 0.709)},  # Right side
    'Changedown': {'top_left': (0.95, 0.408), 'bottom_right': (0.99, 0.439)},  # Right side
    'ChangeUp': {'top_left': (0.95, 0.1429), 'bottom_right': (0.99, 0.174)},  # Right side
    'option1_1': {'top_left': (0.76, 0.200), 'bottom_right': (0.95, 0.260)},  # Right side options
    'option1_2': {'top_left': (0.76, 0.268), 'bottom_right': (0.95, 0.330)},  # Right side options
    'option1_3': {'top_left': (0.76, 0.340), 'bottom_right': (0.95, 0.400)}  # Right side options
}
...
def gen_frames():
    ...
    while True:
        ...
        if not success:
            ...
            # Calculate frame dimensions for buttons alignment
            h, w, _ = frame.shape

            # Draw rectangles from HTML button positions received via Socket.IO
            for button in buttons_positions:
                top_left = (int(button['top_left']['x']), int(button['top_left']['y']))
                bottom_right = (int(button['bottom_right']['x']), int(button['bottom_right']['y']))
                cv2.rectangle(frame, top_left, bottom_right, (255, 255, 255), 2)

            # The original logic for processing hand landmarks remains the same
            ...
            # Capture logic remains unchanged
...
def send_request_and_save(vton_img_path, garm_img_path, output_folder, category):
    ...
    if isinstance(result, list) and len(result) > 0 and 'image' in result[0]:
        ...
        if os.path.exists(image_path):
            ...
            relative_path = "./static/output/image.webp"
...
if __name__ == '__main__':
    socketio.run(app, debug=True)  # Change 8080 to any port you want
